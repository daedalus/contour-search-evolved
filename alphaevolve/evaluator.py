...
Benchmarks candidate algorithms on multiple graph topologies.
Returns score: lower average runtime = higher score.
"""
...
def run_benchmark(fn: Callable, graph: Graph, start: str, goal: str, heuristic) -> List[float]:
    """Run benchmark multiple times, return list of execution times in seconds."""
...
def evaluate_candidate(algorithm_fn: Callable) -> float:
    """
    Evaluate a contour_search implementation.
    Returns score: higher = better (faster).
    Score = 1.0 / mean_normalized_runtime across benchmarks.
    """
    all_times = []
...
def extract_algorithm(code: str) -> Callable:
    """Extract contour_search function from code string."""
    namespace = {}
    exec(code, namespace)
...
def load_candidate_from_file(filepath: str) -> Callable:
    """Load and extract contour_search from a Python file."""
...
if __name__ == "__main__":
    ...
    print(f"Baseline avg runtime: {1.0/score*1000:.2f}ms")