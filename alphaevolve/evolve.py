...
class ProgramDatabase:
    ...
    def add(self, program: Program) -> None:
        ...
        if len(self.programs) > 200:
            self.programs = sorted(self.programs, key=lambda p: p.score, reverse=True)[
                :100
            ]
...
def build_mutation_prompt(
    ...
    for i, p in enumerate(parents):
        label = chr(65 + i)
        parent_strs.append(
            f"--- Algorithm {label} (score: {p.score:.4f}) ---\n{p.code}"
        )
...
def mutate_min_optimization(code: str) -> str:
    ...
    return code.replace("def contour_search(", "def contour_search(")
...
def call_llm(prompt: str, llm_cmd: Optional[str] = None) -> str:
    ...
    if llm_cmd:
        result = subprocess.run(
            llm_cmd.split() + [prompt], capture_output=True, text=True, timeout=60
        )
...
def run_generation(
    ...
    prompt = build_mutation_prompt(parents, db.best_score, db.best_score, instruction)
    ...
    lineage = ",".join(
        chr(65 + parents.index(p)) if p in parents else "?" for p in parents
    )
...
def main():
    ...
    print(f"{'─' * 40}")
    print(
        f"{0:>4} {baseline_score:>10.4f} {'1.000x':>12} {db.diversity:>10.3f}  baseline"
    )
    ...
    for gen in range(1, args.generations + 1):
        ...
        print(
            f"{gen:>4} {prog.score:>10.4f} {ratio:>11.3f}x {db.diversity:>10.3f} {flag}"
        )

    print(f"\n{'=' * 50}")
    print(f"Best score: {best_so_far.score:.4f} vs baseline {baseline_score:.4f}")
    print(
        f"Speedup: {baseline_score / best_so_far.score:.2f}x"
