...
def contour_search(
    ...
    g_score_n = g_score.__getitem__
    g_score_s = g_score.__setitem__

    while key_heap:
        f_key = heappop(key_heap)
        ...
        for node_i in entries:
            ...
            if g == VISITED:
                continue
            g_score_s(node_i, VISITED)

            for nxt_i, wt in nb_idx[node_i]:
                new_g = g + wt
                if new_g < g_score_n(nxt_i):
                    g_score_s(nxt_i, new_g)
                    pred[nxt_i] = node_i
